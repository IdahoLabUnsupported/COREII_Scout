...
from .get_dataset import get_dataset_ner, map_to_shared_tag
...
class TrainTransformersNER:
    ...
    def __setup_model_data(self, dataset, lower_case):
        ...
        #--- Load model and finetune with provided dataset.
        self.dataset_split, self.label_to_id, self.language, self.unseen_entity_set = get_dataset_ner(
            dataset, lower_case=lower_case)
        # Normalize the newly created label mapping.
        self.label_to_id = {map_to_shared_tag(label): idx
        ...
        self.model = transformers.AutoModelForTokenClassification.from_pretrained(
            self.args.transformers_model, config=config, ignore_mismatched_sizes=True)
        ...
        self.optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=self.args.lr, eps=1e-8)
