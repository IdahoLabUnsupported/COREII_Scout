...
SHARED_NER_LABEL = {
    "item" : ["Malware", "Indicator", "System", "Vulnerability"],
    "location": ["LOCATION", "LOC", "location", "Location",'GPE','FAC'],
    "org": ["ORGANIZATION", "ORG", "organization", "Organization", "corporation", "Threat_group"],
    "person": ["PERSON", "PSN", "person", "PER", "Person"],
    "date": ["DATES", "DATE", "DAT", 'YEAR', 'Year', 'Date',"TIME", "TIM", "Hours", "Time"],
    "tactic": ["TACTIC", "tactic"],
    "technique": ["TECHNIQUE", 'technique'],
    #"time": ["TIME", "TIM", "Hours", "Time"], # Added to date entity
    #"artifact": ["ARTIFACT", "ART", "artifact"],
    #"percent": ["PERCENT", "PNT"],
    #"other": ["OTHER", "MISC"],
    #"money": ["MONEY", "MNY", "Price"],
    #"corporation": ["corporation"],  # Wnut 17 # Added to org entity
    #"group": ["group", "NORP"],
    ...
    #"quantity": ['QUANTITY'],
    #"law": ['LAW'],
    #"geopolitical area": ['GPE'], # Added to location entity
    #"work of art": ["WORK_OF_ART", "creative-work"],
    #"facility": ["FAC"], # Added to location entity
    #"language": ["LANGUAGE"],
...
__all__ = ("get_dataset_ner", "map_to_shared_tag", "VALID_DATASET", "SHARED_NER_LABEL")
...
def map_to_shared_tag(tag: str) -> str:
    """
    Map a given NER tag to the normalized tag according to SHARED_NER_LABEL.
    For example, if tag is "B-PER" and "PER" is in SHARED_NER_LABEL['person'],
    the function returns "B-person".
    """
    if tag == 'O':
        return tag
    try:
        prefix, mention = tag.split('-', 1)
    except ValueError:
        return tag  # Unexpected format; return as-is.
    fixed_mention = [k for k, synonyms in SHARED_NER_LABEL.items() if mention in synonyms]
    return f"{prefix}-{fixed_mention[0]}" if fixed_mention else tag
